...
export async function POST(request) {
    try {
        const { email, password, userstatus } = await request.json();
        const hashPassword = await bcrypt.hash(password, 10);
        await dbConnect();
        const user = await UserModel.create({email, userstatus,password: hashPassword})
        if(!user){return handleError(null, "Error created new user")};
        const tokenData = {
            id: user._id,
            email: user.email,
